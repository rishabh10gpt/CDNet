...
def trainValidateSegmentation(args):
    ...
    if args.file_root == 'LEVIR':
        args.file_root = 'ChangeDetection/LEVIR-CD_256_patches'
    elif args.file_root == 'BCDD':
        args.file_root = 'ChangeDetection/BCDD'
    elif args.file_root == 'SYSU':
        args.file_root = 'ChangeDetection/SYSU'
    elif args.file_root == 'CDD':
        args.file_root = 'ChangeDetection/CDD'
    elif args.file_root == 'BCDD-LEVIR' :
        args.file_root = '/home/rishabh/data/BCDD-LEVIR'
    ...
    trainDataset_main = transform.Compose([
        transform.Normalize(mean=mean, std=std),
        transform.Scale(args.inWidth, args.inHeight),
        transform.RandomCropResize(int(7. / 224. * args.inWidth)),
        transform.RandomFlip(),
        transform.RandomExchange(),
        transform.GaussianNoise(),
        transform.GaussianBlur(),
        transform.RandomSwapBands(),
        transform.ToTensor()
    ])

    valDataset = transform.Compose([
        transform.Normalize(mean=mean, std=std),
        transform.Scale(args.inWidth, args.inHeight),
        transform.ToTensor()
    ])
    ...
    if args.pretrain is not None:
        args.pretrain = args.savedir + '/checkpoint.pth.tar'
        if os.path.isfile(args.pretrain):
            ...
            print("=> no pretrained model found at '{}'".format(args.pretrain))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_root', default="BCDD-LEVIR", help='Data directory | BCDD-LEVIR | LEVIR | BCDD | SYSU ')
    ...
    parser.add_argument('--pretrain', default=False, help='Use this checkpoint to continue training | pretrained checkpoint path.')
